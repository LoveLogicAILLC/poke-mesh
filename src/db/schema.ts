...
export const agents = sqliteTable('agents', {
  ...
  status: text('status', { enum: ['active', 'inactive', 'terminated'] })
    .notNull()
    .default('active'),
...
export const messages = sqliteTable('messages', {
  ...
  status: text('status', { enum: ['pending', 'delivered', 'expired', 'failed'] })
    .notNull()
    .default('pending'),
  createdAt: integer('created_at', { mode: 'timestamp' })
    .notNull()
...
export const meshState = sqliteTable('mesh_state', {
  ...
  topology: text('topology', { enum: ['star', 'ring', 'full-mesh'] })
    .notNull()
    .default('star'),
